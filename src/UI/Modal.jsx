  ...
  return (
    !open && (
      ...
      <div onClick={OnCloseHandler} className="w-full h-screen bg-secondary-800 bg-opacity-30 z-40 backdrop-blur-sm fixed top-0 left-0">
        {/* Main Modal */}
        <div className="w-full max-w-lg fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-secondary-0 rounded-lg p-4 shadow-lg transition-all duration-500 ease-in-out">
          {/* Header */}
          <div class="flex-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
            <h3 class="flex-center flex-1 text-xl font-MorabbaBold text-secondary-700 dark:text-white">
              {title}
            </h3>
            <button
